...
void usage(void) 
{
    ...
    printf("[   ./usrPalTest.static open   		/dev/pal0 or /dev/asi0    	]\n");
    printf("[   ./usrPalTest.static close  		/dev/pal0 or /dev/asi0    	]\n");
    ...
    printf("[   ./usrPalTest.static readtofile  /dev/pal0 or /dev/asi0 		]\n");
    printf("\n");
...
static const struct ioctl_table_t ioctl_table[] = {
	{
		.cmd    = PAL_GET_PING_ADDR,
		.cmdStr = "PAL_GET_PING_ADDR",
	},
	{
		.cmd    = PAL_SET_PING_ADDR,
		.cmdStr = "PAL_SET_PING_ADDR",
	},
	{
		.cmd    = PAL_GET_RX_LEN,
		.cmdStr = "PAL_GET_RX_LEN",
	},
};
...
/* 测试用例1: pal_open设备测试  */
void pal_open_test(const char *pathname) 
{
	int ret = -1;
	int fd;	
	...
	printf("open %s Test Success!\n",pathname);
	return;
}

/* 测试用例2: pal_close设备测试  */
void pal_close_test(const char *pathname) 
{
	int ret = -1;
	int fd;
	...
	ret = close(fd);
	if (ret){
		printf("close %s Failed! ret = 0x%x.\n",pathname,ret);
		return;
	}		

	printf("close %s Test Success!\n",pathname);
	return;
}
...
/* 测试用例4: pal_read 阻塞读DMA ping/pong数据测试  */
...
/* 测试用例5: 测试获取/dev/pal0的ping地址和ping内存数据长度，并做mmap，取数据。 */
...
/* 测试用例6: pal_read 阻塞读DMA ping/pong数据测试  */
void pal_readtofile_test(const char *pathname) 
{
	int ret = -1;
	int fd;
	unsigned char buf[0x20000];
...
{    
    ...
    if(strcmp(argv[1],"open") == 0)
    	pal_open_test(argv[2]);
    else if (strcmp(argv[1],"close") == 0)
    	pal_close_test(argv[2]);
